...
public class CreateRideGUI extends JFrame {
	...
	public CreateRideGUI(Driver driver) {
		...
		datesWithEventsCurrentMonth=facade.getThisMonthDatesWithRides("a","b",UtilDate.newDateXMLGregorianCalendar(jCalendar.getDate()));		
		...
		if (error!=null) 
			...
			try {
				...
				float price = Float.parseFloat(jTextFieldPrice.getText());
					...
					Ride r=facade.createRide(fieldOrigin.getText(), fieldDestination.getText(), UtilDate.newDateXMLGregorianCalendar(UtilDate.trim(jCalendar.getDate())), inputSeats, price, driver.getEmail());
					jLabelMsg.setText(ResourceBundle.getBundle("Etiquetas").getString("CreateRideGUI.RideCreated"));
				} catch (RideAlreadyExistException_Exception e1) {
				jLabelMsg.setText(e1.getMessage());
				} catch (RideMustBeLaterThanTodayException_Exception e1) {
